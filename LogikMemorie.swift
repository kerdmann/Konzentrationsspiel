...
class LogikMemorie {
    ...
    func eineKarteWurdeUmgedreht(Kartennummer positionAktuelleKarte: Int){
        if !(KartenPassendZuButtons[positionAktuelleKarte].istGefunden){
            if !(positionAktuelleKarte == offeneKarte) {
                if offeneKarte == nil {
                    ...
                    if KartenPassendZuButtons[offeneKarte!].idNummer == KartenPassendZuButtons[positionAktuelleKarte].idNummer {
